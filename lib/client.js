 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    for (let i of owner) {
                        Void.sendMessage(i + "@s.whatsapp.net", { text: `_السلام عليكم..._\n\n_قبل الأمر : [ ${prefix} ]_\n_الأوامر : ${events.commands.length}_\n_الوضع: ${Config.WORKTYPE}_\n_الأصدار:- 0.0.5_\n_براش:- ${Config.BRANCH}_\n_الثيم: ${Config.LANG}_\n_المطور:- ${process.env.OWNER_NAME}_\n\n التواصل مع المطور \n https://api.whatsapp.com/send?phone=+967770500831` })
                    }
                }
